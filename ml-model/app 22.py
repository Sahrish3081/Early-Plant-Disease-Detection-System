...
async def predict(file: UploadFile = File(...)):
    ...
    top_k = sorted(enumerate(preds), key=lambda t: t[1], reverse=True)[:3]
    top_predictions = []
    for i, p in top_k:
        c, d = split_label(class_names[i])
        top_predictions.append({"label": f"{c} — {d}" if d else c, "prob": round(float(p) * 100, 2)})

    # Low-confidence flag for real-world (blurry / off-topic) photos
    low_confidence = confidence < 60.0

    print(f"Prediction: {label}  conf={confidence:.2f}%  low_conf={low_confidence}")
