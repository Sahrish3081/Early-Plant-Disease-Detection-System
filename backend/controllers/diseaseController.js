...
const detectDisease = async (req, res, next) => {
  try {
    ...
    const response = {
      ...
      diseaseName: result.diseaseName,       // e.g. "Tomato Mosaic virus"

      // ── NEW: transparency fields from the model ──────────────────────────
      uncertain: result.uncertain,                   // true => show warning banner
      uncertaintyMessage: result.uncertaintyMessage, // human-readable reason
      topPredictions: result.topPredictions,         // [{label, prob}, ...] top-3
      ...
      disease: {
        ...
        // honest alias also available if you rename the UI label:
        attentionCoveragePercent: result.attentionCoveragePercent,
      },

      annotations: result.annotations,        // Boxes for canvas (current red-box view)
      // annotatedImage: result.annotatedImage, // (optional) Python's Grad-CAM heatmap overlay
      info: result.info,                      // Description, causes, treatment, prevention
    };
